...
lazy_static! {
    pub static ref GIC_FEATURES: GicFeatures = {
        ...
        let nr_lrs = unsafe {
            labeling::unlabeled();
            ICH_VTR_EL2.get_masked_value(ICH_VTR_EL2::LIST)
        } as usize;
        trace!("nr_lrs (LIST) {}", nr_lrs);
        let id = unsafe {
            labeling::unlabeled();
            ICH_VTR_EL2.get_masked_value(ICH_VTR_EL2::ID)
        };
        ...
        let pre = unsafe {
            labeling::unlabeled();
            ICH_VTR_EL2.get_masked_value(ICH_VTR_EL2::PRE)
        } + 1;
        let nr_aprs = (1 << (pre - 5)) - 1;
        trace!("pre {}, nr_aprs {}", pre, nr_aprs);
        let pri = unsafe {
            labeling::unlabeled();
            ICH_VTR_EL2.get_masked_value(ICH_VTR_EL2::PRI)
        } + 1;
...
fn set_lr(i: usize, val: u64) {
    match i {
        ...
        10 => unsafe {
            labeling::unlabeled();
            ICH_LR10_EL2.set(val)
        },
        11 => unsafe {
            labeling::unlabeled();
            ICH_LR11_EL2.set(val)
        },
        12 => unsafe {
            labeling::unlabeled();
            ICH_LR12_EL2.set(val)
        },
        13 => unsafe {
            labeling::unlabeled();
            ICH_LR13_EL2.set(val)
        },
        14 => unsafe {
            labeling::unlabeled();
            ICH_LR14_EL2.set(val)
        },
        15 => unsafe {
            labeling::unlabeled();
            ICH_LR15_EL2.set(val)
        },
        _ => {}
    }
}
...
pub fn save_state(vcpu: &mut VCPU<Context>) {
    ...
    gic_state.ich_vmcr_el2 = unsafe {
        labeling::unlabeled();
        ICH_VMCR_EL2.get()
    };
    gic_state.ich_hcr_el2 = unsafe {
        labeling::unlabeled();
        ICH_HCR_EL2.get()
    };
    gic_state.ich_misr_el2 = unsafe {
        labeling::unlabeled();
        ICH_MISR_EL2.get()
    };
    ...
    unsafe {
        labeling::unlabeled();
        ICH_HCR_EL2.set(gic_state.ich_hcr_el2 & !ICH_HCR_EL2_EN_BIT)
    };
}
...
pub fn receive_state_from_host(id: usize, vcpu: usize, run: &Run) -> Result<(), Error> {
    ...
    gic_state.ich_lr_el2[..nr_lrs].copy_from_slice(
        &unsafe {
            labeling::unlabeled();
            run.entry_gic_lrs()
        }[..nr_lrs],
