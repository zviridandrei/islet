...
pub fn set_event_handler(rsi: &mut RsiHandle) {
    ...
    listen!(rsi, rsi::PSCI_CPU_ON, |_arg, ret, _rmm, rec, run| {
        ...
        let target_mpidr = get_reg(realmid, vcpuid, 1)? as u64;
        let entry_addr = get_reg(realmid, vcpuid, 2)?;

        if !rd.addr_in_par(entry_addr) {
            set_reg(realmid, vcpuid, 0, PsciReturn::INVALID_ADDRESS)?;
            ret[0] = rmi::SUCCESS_REC_ENTER;
            return Ok(());
        }
        ...
        if target_index == rec.vcpuid() {
            set_reg(realmid, vcpuid, 0, PsciReturn::ALREADY_ON)?;
            ret[0] = rmi::SUCCESS_REC_ENTER;
            return Ok(());
        }
    ...
    listen!(rsi, rsi::PSCI_FEATURES, |_arg, ret, _rmm, rec, _run| {
        ...
        let feature_id = get_reg(realmid, vcpuid, 1)?;
...
pub fn complete_psci(
    ...
    let realmid = caller.realmid()?;
    let caller_vcpuid = caller.vcpuid();
    let target_vcpuid = target.vcpuid();

    let target_mpidr = get_reg(realmid, caller_vcpuid, 1)? as u64;
    if MPIDR::from(target_mpidr).index() != target_vcpuid {
        return Err(Error::RmiErrorInput);
    }

    let command = get_reg(realmid, caller_vcpuid, 0)?;
    match command {
        rsi::PSCI_CPU_ON => {
    ...
    set_reg(realmid, caller_vcpuid, 0, psci_ret)?;
    set_reg(realmid, caller_vcpuid, 1, 0)?;
    set_reg(realmid, caller_vcpuid, 2, 0)?;
    set_reg(realmid, caller_vcpuid, 3, 0)?;
