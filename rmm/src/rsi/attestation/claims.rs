...
impl<T: Into<Value>> From<Claim<T>> for (Value, Value){
    fn from(value: Claim<T>) -> Self {
        (Value::Integer(value.label.into()),
        value.value.into())
    }
}
...
impl RealmClaims {
    pub fn init(
        ...
        key_pub: &[u8],
        key_pub_hash_algo: String,) -> RealmClaims
    {
        let challenge_claim: Claim<Challenge> =
            Claim {
        ...
        let rim: Claim<RIM> =
            Claim {
                ...
                value: MeasurementEntry(measurements[MEASUREMENTS_SLOT_RIM],
                                       measurement_size),
            };
