...
fn get_token_part(
    ...
    let realmid = context.realmid()?;
    let hash_algo = get_granule_if!(context.owner()?, GranuleState::RD)?
        .content::<Rd>()
...
pub fn set_event_handler(rsi: &mut RsiHandle) {
    listen!(rsi, ATTEST_TOKEN_INIT, |_arg, ret, _rmm, rec, _| {
        ...
        set_reg(realmid, vcpuid, 1, attestation::MAX_CCA_TOKEN_SIZE)?;
    ...
    listen!(rsi, ATTEST_TOKEN_CONTINUE, |_arg, ret, _rmm, rec, _| {
        ...
        let pa_offset = get_reg(realmid, vcpuid, 2)?;
        let buffer_size = get_reg(realmid, vcpuid, 3)?;
        ...
        let attest_pa: usize = crate::realm::registry::get_realm(realmid)
            .ok_or(Error::RmiErrorOthers(NotExistRealm))?
            .lock()
            .page_table
            .lock()
        ...
        if token_left == 0 {
            ...
            set_reg(realmid, vcpuid, 0, INCOMPLETE)?;
        }

        set_reg(realmid, vcpuid, 1, token_part.len())?;
